...
import {
  Bed, Bath, Car, MapPin, Star, Heart,
...
export default function PropertyDetailPage() {
  ...
  const { mutate: toggleFav, isPending: favPending } = useMutation({
    mutationFn: () => favData?.data?.isFavourited
      ? api.delete(`/favourites/${id}`)
      : api.post(`/favourites/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries(['fav-check', id])
      toast.success(favData?.data?.isFavourited ? 'Removed from favourites' : 'Saved to favourites')
    },
  })
  ...
  if (!property) return (
    <div className="text-center py-20">
      ...
      <Link to="/listings" className="text-primary-800 mt-2 inline-block">← Back to listings</Link>
    </div>
  )
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Back */}
      <Link to="/listings" className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="flex items-center gap-1.5 text-gray-500 mt-1 text-sm">
                  <MapPin size={14} />
                  <span>{[property.street_address, property.suburb, property.state, property.postcode].filter(Boolean).join(', ')}</span>
                </div>
              </div>
              <Badge variant={property.status === 'available' ? 'green' : 'red'} className="flex-shrink-0">
                {property.status}
              </Badge>
            </div>
                  ...
                  <div key={r._id} className="flex gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-sm">{r.reviewerId?.name}</span>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-5">
            ...
            {token && user?.role === 'tenant' && property.status === 'available' && (
              <div className="space-y-2">
                <Button
                  fullWidth
                  onClick={() => navigate(`/dashboard/applications?propertyId=${property._id}`)}
                >
                  Apply Now
                </Button>
                <button
                ...
                >
                  <Heart size={15} fill={favData?.data?.isFavourited ? 'currentColor' : 'none'} />
